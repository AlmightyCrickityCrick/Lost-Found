...
object ApolloClientService {
    ...
    private val apolloClient =
        ApolloClient.Builder().serverUrl(URL).okHttpClient(okHttpClient).build()
    ...
    suspend fun register(username: String, password: String): String? {
        val response =
            apolloClient.mutation(CreateUserMutation(username, HashingService.hashString(password)))
                .execute()
    ...
    suspend fun login(username: String, password: String): String? {
        val response =
            apolloClient.mutation(LoginMutation(username, HashingService.hashString(password)))
                .execute()
    ...
    fun getCoords(str: String): LatLng {
        if ('|' in str) {
            ...
            var long: Double =
                str.subSequence(str.indexOf('|') + 1, str.length).toString().toDouble()
    ...
    suspend fun getAnnDetails(id: Int): AnnDetails? {
        ...
        if ((response?.data != null) && (response?.data?.announcement != null)) {
            ...
            if (r != null) return AnnDetails(
                r.id.toInt(),
                r.title,
                r.content,
                User(
                    r.userProfile.id.toInt(),
                    "${r.userProfile.user.firstName} ${r.userProfile.user.lastName}",
                    R.drawable.ic_account,
                    r.userProfile.rating,
                    r.userProfile.publicKey
                ),
                getDetailsTags(r.tags),
                r.streetName,
                r.image
            )
        }
    ...
    suspend fun setUserInfo(
        ...
        val response = authorizedApolloClient.mutation(
    ...
    private class AuthorizationInterceptor(val token: String) : Interceptor {
        override fun intercept(chain: Interceptor.Chain): Response {
            val request =
                chain.request().newBuilder().addHeader("Authorization", "JWT $token").build()
    ...
    suspend fun createAnnouncement(
        ...
        title: String,
        usrId: Int
    ): Result<AnnouncementCreationRequestResult> {
...
////            }}
        ...
        val response = authorizedApolloClient.mutation(
            CreateNewAnnouncementMutation(
                type,
                content,
                Optional.present(image?.toUpload("image/jpg")),
                coords,
                adress,
                Optional.present(0),
                tag,
...
//        } else response= apolloClient.mutation(CreateNewAnnouncementMutation(type, content, Optional.present(null), location, Optional.present(0), tag, title, usrId)).execute()
    ...
    suspend fun getOTP(id: Int, code: String): String? {
        var response = apolloClient.mutation(
            GetOTPResultMutation(
                Optional.present(id),
                Optional.present(code)
            )
        ).execute()
    ...
    suspend fun createChat(id: Int, name: String, key: String): String? {
        var response = authorizedApolloClient.mutation(
            CreateChatMutation(
                id.toString(),
        ...
        if (response.errors.isNullOrEmpty()) {
            return response.data!!.createChat!!.chat!!.id
        }
        return null
    }


    suspend fun acceptChat(id: Int): Pair<String, String>? {
        var response = authorizedApolloClient.mutation(
            AcceptChatMutation(
                id.toString()
            )
        ...
        if (response.errors.isNullOrEmpty()) {
            return Pair(
                response.data!!.acceptChat!!.id.toString(),
                response.data!!.acceptChat!!.key.toString()
            )
        }
        return null
    }

    suspend fun getAllChats(myId:String): ArrayList<Contact> {
        var tmp = ArrayList<Contact>()
        var response = authorizedApolloClient.query(GetAllChatsQuery()).execute()
        if (response.errors.isNullOrEmpty() && !response.data!!.chats.isNullOrEmpty()) {
            for (c in response.data!!.chats!!){
                for (p in c!!.participants!!){
                    if(p.id == myId) continue
                    if(c!=null)tmp.add(Contact(c.id, User(p.id.toInt(), c.name, R.drawable.ic_account),"Hello, I fund something that belongs to you", "23:59 pm", c.state), )
                }
            }
        }
        return tmp
